...
exports.createMenuListing = async (req, res) => {
  try {
    ...
    const newMenuListingData = { pagename, alt, priority: parseInt(priority) };

    // If photo is uploaded, add the photo filename to the data
    if (req.file) {
      newMenuListingData.photo = req.file.filename;
    }

    // Increment priorities of existing menu listings
    await MenuListing.updateMany({ priority: { $gte: parseInt(priority) } }, { $inc: { priority: 1 } });

    // Create a new MenuListing and save to database
    const newMenuListing = new MenuListing(newMenuListingData);
