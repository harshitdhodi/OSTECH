...
const MissionForm = () => {
    ...
    const [initialPhotoTitles, setInitialPhotoTitles] = useState([]); // For existing photos
    const [photoTitles, setPhotoTitles] = useState([]); // For new photos
    ...
    const fetchMission = async () => {
        try {
            const response = await axios.get('/api/mainMission/getMission', { withCredentials: true });
            ...
            setInitialPhotoTitles(mission.titles || []);
    ...
    const handleSubmit = async (e) => {
        ...
        const combinedTitles = [...initialPhotoTitles, ...photoTitles];
        combinedTitles.forEach((t) => {
        ...
        try {
            await axios.put('/api/mainMission/updateMission', formData, { withCredentials: true });
            notify();
            setPhoto([]);
            setPhotoAlts([]);
            await fetchMission();
    ...
    const handleInitialTitleChange = (e, index) => {
        const newPhotoTitles = [...initialPhotoTitles];
        newPhotoTitles[index] = e.target.value;
        setInitialPhotoTitles(newPhotoTitles);
    };

    const handleNewTitleChange = (e, index) => {
    ...
    const handleDeleteInitialPhoto = (e, photoFilename, index) => {
        e.preventDefault();
        axios.delete(`/api/mainMission/image/${photoFilename}/${index}`, { withCredentials: true })
            .then(response => {
                        ...
                        {initialPhotos.map((photo, index) => (
                            <div key={index} className="relative w-56">
                                ...
                                <button
                                    ...
                                    className="absolute top-4 right-2 bg-red-500 text-white rounded-md p-1"
                                >
                                    &times;
                                </button>
                                ...
                                <label htmlFor={`title-${index}`} className="block mt-2">
                                    ...
                                    <input
                                        type="text"
                                        id={`title-${index}`}
                                        value={initialPhotoTitles[index] || ''}
                                        onChange={(e) => handleInitialTitleChange(e, index)}
                                        className="w-full p-2 border rounded"
                                    />
                                </label>
                            </div>
                        ...
                        <div key={index} className="relative w-56 mb-2">
                            ...
                            <label htmlFor={`title-new-${index}`} className="block mt-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleNewTitleChange(e, index)}
                                    className="w-full p-2 border rounded"
                                />
                            </label>
                        </div>
