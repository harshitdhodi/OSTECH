...
const ProductsTable = () => {
  ...
  }, [pageIndex, pageSize, filteredProducts]);
  ...
  const fetchProducts = async (pageIndex) => {
    ...
    try {
      const response = await axios.get(`/api/product/getAllProducts`, { 
        params: {
          page: pageIndex + 1, // Adding 1 since backend typically uses 1-based indexing
          limit: pageSize
        },
        withCredentials: true 
      });
      
      // Update to use the paginated data from backend
      ...
      // Assuming backend returns total count of products
    ...
    <div className="p-4 overflow-x-auto">
          ...
          <tbody {...getTableBodyProps()}>
            {paginatedData.map((product, index) => {
              ...
              return (
                ...
                >
                  {row.cells.map((cell) => (
                    <td key={cell.column.id} className="py-2 px-4">
                      {cell.render("Cell")}
                    </td>
                  ))}
                </tr>
      ...
      <div className="mt-4 flex justify-center items-center gap-2">
        <button 
          ...
          className="px-3 py-1 bg-slate-700 text-white rounded hover:bg-slate-900 transition disabled:bg-gray-300 disabled:cursor-not-allowed"
        >
          First
        </button>
        <button 
        ...
        <div className="flex items-center gap-1">
          {pageCount > 0 && Array.from({ length: Math.min(pageCount, 5) }, (_, i) => {
            ...
            let startPage = Math.max(0, Math.min(pageIndex - 2, pageCount - 5));
