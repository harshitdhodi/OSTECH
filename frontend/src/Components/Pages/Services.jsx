...
const ServicesTable = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/services/getService?page=${pageIndex + 1}`, { withCredentials: true });
  ...
  const deleteService = async (slugs) => {
    try {
      const response = await axios.delete(`/api/services/deleteService?slugs=${slugs}`, { withCredentials: true });
  ...
  const fetchHeadings = async () => {
    try {
      const response = await axios.get('/api/pageHeading/heading?pageType=service', { withCredentials: true });
  ...
  const saveHeadings = async () => {
    try {
      await axios.put('/api/pageHeading/updateHeading?pageType=service', {
        pagetype: 'service',
        heading,
        subheading,
  ...
  function exportServices() {
    axios.get('/api/services/exportService', { responseType: 'blob', withCredentials: true })
      .then(response => {
  ...
  const onFileUpload = async () => {
    ...
    try {
      const response = await axios.post('/api/services/importService', formData, {
        headers: {
