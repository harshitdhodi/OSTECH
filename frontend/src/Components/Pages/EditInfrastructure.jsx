...
const EditInfrastructure = () => {
  ...
  const fetchInfrastructure = async () => {
    try {
      const response = await axios.get(`/api/infrastructure/getInfrastructureById?id=${id}`, { withCredentials: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`/api/infrastructure/updateInfrastructure?id=${id}`, formData, {
        headers: {
  ...
  const handleDeleteInitialPhoto = (e, photoFilename,index) => {
    e.preventDefault();
    axios.delete(`/api/infrastructure/${id}/image/${photoFilename}/${index}`, { withCredentials: true })
    .then(response => {
        ...
        <div className="flex flex-wrap gap-4">
          {initialPhotos.map((photo, index) => (
            <div key={index} className="relative w-56">
              <img
                src={`/api/infrastructure/download/${photo}`}
                alt={`Photo ${index + 1}`}
