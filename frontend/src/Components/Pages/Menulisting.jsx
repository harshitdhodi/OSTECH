...
const MenuListingTable = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "photo",
        Cell: ({ value }) => {
          return <img src={`/api/logo/download/${value}`} alt="Menu" className="h-20 w-32 object-cover " />;
        },
        disableSortBy: true,
      },
  ...
  const deleteMenuListing = async (id) => {
    try {
      const response = await axios.delete(`/api/menulisting/deleteMenulisting?id=${id}`, { withCredentials: true });
