...
const InfrastructureTable = () => {
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({ value }) => {
          ...
          return firstImage ? <img src={`/api/infrastructure/download/${firstImage}`} alt="Infrastructure" className="w-32 object-cover h-20" /> : null;
        },
        disableSortBy: true,
      },
      {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/infrastructure/getInfrastructure`, { withCredentials: true });
  ...
  const deleteInfrastructure = async (id) => {
    try {
      const response = await axios.delete(`/api/infrastructure/deleteInfrastructure?id=${id}`, { withCredentials: true });
  ...
  const fetchHeadings = async () => {
    try {
      const response = await axios.get('/api/pageHeading/heading?pageType=infrastructure', { withCredentials: true });
  ...
  const saveHeadings = async () => {
    try {
      await axios.put('/api/pageHeading/updateHeading?pageType=infrastructure', {
