...
import { useGetInfographicsQuery, useDeleteInfographicMutation } from '@/slice/infographic'; // Assuming you have RTK Query set up
import { FaEdit, FaTrashAlt } from "react-icons/fa";
import { Link } from 'react-router-dom';

const InfographicsTable = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const { data, error, isLoading } = useGetInfographicsQuery(currentPage); // Fetch data based on page number
  const [deleteInfographic] = useDeleteInfographicMutation(); // Use RTK Query mutation for delete
            ...
            <tr key={infographic.id} className="border-b">
              ...
              <td className="px-4 py-2">
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: infographic.description }}
                />
              </td>
              <td className="px-4 py-2 text-black">
                <img src={`/api/image/download/${infographic.photo}`} alt={infographic.title} className="w-16 h-16 object-contain text-black" />
              </td>
              <td className="px-4 md:flex justify-center mt-5 items-center py-2">
                <Link to={`/edit-infographic-form/${infographic._id}`}>
